...
\begin{lemma}[Integrality Preservation]\label{lem:integrality-preservation}
Let a prefix end in the affine form $x_t(m) = 6(A_t m + B_t) + \delta_t$. Suppose the next token $T$ (with parameters $\alpha_p, k^{(p)}, \delta_T$) is admissible, meaning the router condition holds:
\[ A_t m + B_t \equiv r_{t+1} \pmod 3, \]
where $r_{t+1}$ is the router index required by $T$. Then the updated intercept
\[ B_{t+1} = \frac{2^{\alpha_p}}{3}(B_t - r_{t+1}) + k^{(p)} \]
is guaranteed to be an integer.
\end{lemma}

\begin{proof}
The admissibility condition $A_t m + B_t \equiv r_{t+1} \pmod 3$ implies $B_t - r_{t+1} \equiv -A_t m \pmod 3$. Since $A_t$ is a power of 2 (and thus not divisible by 3) and $m$ is chosen from a class where this congruence holds, the term $(B_t - r_{t+1})$ is divisible by 3. Thus, the division by 3 is exact in $\mathbb{Z}$.
\end{proof}
...
\end{proof}
\begin{remark}[Arithmetic Mechanism of Router Stability]
The bound $S^\ast \ge 1 + \sum_{i<t} \alpha_i$ is not arbitrary. It works because fixing $m \pmod{2^{S^\ast}}$ freezes the term $A_t m$ modulo $2 \cdot A_t = 2^{1+\sum \alpha_i}$. Since the router index is determined by
\[ j_{t+1} = \left\lfloor \frac{x_t}{6} \right\rfloor \bmod 3 = (A_t m + B_t) \bmod 3, \]
stabilizing $A_t m$ modulo a sufficiently large power of 2 (specifically one that is coprime to 3) is sufficient to fix the value modulo 3 when combined with the admissibility constraints. The formal proof (see Coq \texttt{lem\_routing\_compatibility\_Sstar}) verifies this by induction.
\end{remark}
...
\begin{enumerate}
  ...
  \item \textbf{Solve $m$-class.}
    In the congruence regime, solve $a^{(p)}m\equiv r^{(p)}\pmod{M_K}$ for $m$; in the pinning regime, any $m$ works.
    Intersect this solution class with the routing class from Lemma~\ref{lem:routing-compat}.
    \emph{(Note: These classes are guaranteed to have a non-empty intersection by the Chinese Remainder Theorem for powers of two, because the base witness at $M_3=24$ already ensures consistency at the lower modulus.)}
    \begin{remark}[Consistency of Constraints]
      In Step 5, we intersect the routing constraint $m \equiv m_{\text{route}} \pmod{2^{S^\ast}}$ with the lifting constraint $m \equiv m_{\text{lift}} \pmod{2^{K-\alpha}}$.
      These constraints are guaranteed to be compatible (have a non-empty intersection) by the Chinese Remainder Theorem for prime powers. Why? Because both constraints are derived from the same base witness $W_3$ at $M_3=24$. The base witness ensures the conditions are met at the lower modulus (min$(S^\ast, K-\alpha)$), and the lifting process preserves this validity at higher powers of 2.
\end{remark}
...
\begin{remark}[Equivalence of Witnesses and Orbits]
Theorem~\ref{thm:odd-layer-convergence} establishes that the existence of a lifted witness implies convergence to $1$ (Sufficiency). Conversely, Corollary~\ref{cor:completeness} (Algebraic Completeness) ensures that every valid preimage under $U$ corresponds to a row in the unified table. Therefore, any integer $x$ that reaches $1$ does so via a unique trajectory that is exactly described by some admissible word $W$. The search for a witness is thus equivalent to the search for the orbit itself; our method simply performs this search in the complete inverse space.
\end{remark}
...
\label{thm:cycle-location-bound}
...
\begin{remark}[Arithmetic Sterilization of ``Ghost'' Fixed Points at $p=0$]
While Theorem~\ref{thm:cycle-location-bound} restricts cycles to a neighborhood of $-v_W$, at low slopes ($p=0$) this neighborhood may appear large enough to contain integers. However, arithmetic constraints (admissibility and the structure of $\mathbb{Z}_{\text{odd}}$) can ``sterilize'' these regions.

Consider the row $\omega_0$ at $p=0$ (Type \texttt{oe}, $\alpha=3$).
\begin{itemize}
    \item \textbf{Geometric Trap:} The affine parameters yield a repulsive fixed point at $x^* \approx 6.2$. This suggests a potential cycle near the integers $6$ or $7$.
    \item \textbf{Arithmetic Exclusion:}
    \begin{enumerate}
        \item The integer $6$ is even (and $0 \pmod 3$), hence it is not in the domain of the odd map $U$.
        \item The integer $7$ is odd, but $7 \equiv 1 \pmod 6$, placing it in family $\mathbf{e}$. The row $\omega_0$ requires inputs from family $\mathbf{o}$ ($x \equiv 5 \pmod 6$). Thus, $7$ is \emph{algebraically invisible} to this operator.
    \end{enumerate}
\end{itemize}
The nearest admissible integer is $5$, but the map repels $5$ away from the trap ($5 \mapsto 13$). Thus, the ``ghost'' fixed point at $6.2$ is dynamically empty. This illustrates how the \textbf{Admissibility Mask} filters out geometric attractors that do not align with the residue structure of the map.
\end{remark}

\begin{remark}[Arithmetic Exclusion and Dynamical Repulsion at $p=0$]
While Theorem~\ref{thm:cycle-location-bound} restricts cycles to a neighborhood of $-v_W$, at low slopes ($p=0$) this neighborhood may appear large enough to contain integers. However, a combination of arithmetic constraints and strong local repulsion clears these regions.

Consider the row $\Omega_0$ at $p=0$ (Type \texttt{oo}, $\alpha=5$).
\begin{itemize}
    \item \textbf{Geometric Trap:} The affine parameters yield a repulsive fixed point at $x^* \approx 5.2$. This suggests a potential cycle near the integers $5$ or $6$.
    \item \textbf{Exclusion Analysis:}
    \begin{enumerate}
        \item The integer $6$ is even (and $0 \pmod 3$), hence it is not in the domain of the odd map $U$. It is structurally excluded.
        \item The integer $5$ satisfies the family constraint ($\Omega_0$ requires inputs from $\mathbf{o}$, i.e., $x \equiv 5 \pmod 6$). However, the map at this point is strongly repulsive: applying $\Omega_0$ ($m=0$) yields $x' = 192(0) + 53 = 53$.
    \end{enumerate}
\end{itemize}
The orbit is violently ejected ($5 \to 53$) away from the fixed point $5.2$. Thus, the ``ghost'' fixed point contains no cycles: invalid integers are arithmetically invisible, and valid integers are dynamically transient.
\end{remark}
...
% Algorithm 4: Search for a parent without a fixed p limit
\begin{algorithm}[H]
\caption{Reverse-One-Step-Unbounded-p\texorpdfstring{$(y)$}{(y)}}
...
\For{each row $(s,j,\alpha,\beta,c,\delta)$ with $\delta \equiv y \pmod{6}$}
  ...
  \For{$p \gets 0,1,2,\ldots$}
    \If{$\beta \cdot 64^{p} + c > 9T$}
      \State \textbf{break} \Comment{early stop for this row (monotonicity)}
    \EndIf
...
% Algorithm 5: Helper to check a specific lift p
\begin{algorithm}[H]
...
\For{each row $(s,j,\alpha,\beta,c,\delta)$ with $\delta\equiv y\ (\mathrm{mod}\ 6)$}
  \State $k_p \gets (\beta\,64^{\,p}+c)/9$
  \If{$k_p \notin \mathbb{Z}$} \textbf{continue} \EndIf
  \State $num \gets (y-\delta)/6 - k_p$
  \State $den \gets 2^{\alpha+6p}$
  \If{$num < 0$ \textbf{or} $num \bmod den \neq 0$} \textbf{continue} \EndIf
  \State $m \gets num / den$
...
% Algorithm 6: Full chain builder with padding fallback
\begin{algorithm}[H]
...
\While{$y\neq \text{stop}$}
  ...
  \If{$x$ is \textbf{fail}}
    \State \Comment{Fallback: try 1-layer same-family padding}
    \State $found \gets \mathbf{false}$
    \For{each padding gadget $G$ compatible with family of $y$}
        \For{each row $R$ admissible after $G$}
            \State Attempt to solve composite step $y \leftarrow R \leftarrow G \leftarrow x$
            \If{solution $m \ge 0$ found}
                \State $x \gets \text{parent}(m)$; $found \gets \mathbf{true}$
                \State \textbf{break}
            \EndIf
        \EndFor
        \If{$found$} \textbf{break} \EndIf
    \EndFor
    \If{\textbf{not} $found$} \State \Return \textbf{fail} \EndIf
  \EndIf
...
\href{https://doi.org/ DOI: 10.5281/zenodo.17899353}{Zenodo DOI: 10.5281/zenodo.17899353} and mirrored at
