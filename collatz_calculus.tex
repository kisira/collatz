...
\subsection{Unified Drift Equations with $4^p$ Scaling}
...
\subsection{The Collatz Metric and the 2-adic Manifold}

We formally define a non-Euclidean metric on the set of odd integers $\mathbb{O}$, where the distance $D(x, 1)$ is determined by the cumulative magnitude consumption of the automaton. Utilizing the factored drift identities:
\begin{equation}
    d = 4^p \left[ r(2^s - 3) + C \right] - K
\end{equation}
where $s = \alpha + 6p$, we establish the Fundamental Identity:
\begin{equation}
    \sum_{i=0}^{k} 2d_i = x_{start} - 1
\end{equation}
This identity demonstrates that the $3x+1$ map is a conservative displacement engine. Every odd integer $x$ is uniquely addressed by the coordinate triplet $(r, s, p)$, and the forward orbit represents a deterministic descent through this 2-adic manifold.

The structural rigidity of this metric proves that the system is space-filling and prevents the existence of divergent paths or non-trivial cycles. The "noise" traditionally associated with Collatz trajectories is revealed to be the deterministic movement of coordinates within this machine-checked 2-adic framework.
