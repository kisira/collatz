...
\begin{proposition}[Row-consistent reversibility]
\label{prop:row-consistent-reverse}
Fix a unified $p{=}0$ row with parameters $(s,j,\alpha,k,\delta)$ and type
\texttt{ee}, \texttt{eo}, \texttt{oe}, or \texttt{oo}, written in the form
\[
x' \;=\; 6\bigl(2^{\alpha} m + k\bigr) + \delta,
\qquad \delta=\begin{cases}1,&\texttt{*e}\\[2pt] 5,&\texttt{*o.}\end{cases}
\]
Let $x'$ be an odd integer with $x'\equiv \delta \pmod 6$. Define
\[
m \;:=\; \frac{(x'-\delta)/6 - k}{2^{\alpha}}.
\]
If $m\in\mathbb{Z}_{\ge 0}$, set the \emph{candidate predecessor}
\[
x \;:=\; 18\,m \;+\; 6j \;+\; p_6,
\qquad
p_6=\begin{cases}1,& s=\mathrm e,\\ 5,& s=\mathrm o.\end{cases}
\]
Then $x$ is odd with $s(x)=s$, its indices satisfy
$m=\lfloor x/18\rfloor$ and $j=\lfloor x/6\rfloor\bmod 3$, and the step is certified:
\[
3x'+1 \;=\; 2^{\alpha}\,x
\qquad\Longleftrightarrow\qquad
U(x')=x.
\]
In particular, whenever $m$ computed above is an integer, this row furnishes a \emph{legal} inverse step from $x'$ to $x$.
\end{proposition}

\begin{proof}[Proof sketch]
By construction $x'=6(2^\alpha m+k)+\delta$ and $x=18m+6j+p_6$, so
$3x'+1=18\cdot 2^\alpha m + (18k+3\delta+1)=2^\alpha(18m+6j+p_6)=2^\alpha x$,
using the row design relation $18k+3\delta+1=2^\alpha(6j+p_6)$.
Thus $U(x')=x$. The formulas for $x$ give $s(x)=s$, $m=\lfloor x/18\rfloor$ and
$j=\lfloor x/6\rfloor\bmod 3$ by direct check.
\end{proof}

\begin{remark}[Uniqueness via $U$]
For any odd $x'$, the value $U(x')=\dfrac{3x'+1}{2^{\nu_2(3x'+1)}}$ is uniquely determined.
If one computes $x:=U(x')$ first and then extracts $(s,j,m)$ from $x$,
the \emph{corresponding} row in the unified table necessarily reproduces $x'$.
Proposition~\ref{prop:row-consistent-reverse} simply packages this as a row-by-row inverse test: only rows for which $m$ is integral are valid predecessors for that specific $x'$.
\end{remark}

\begin{example}[Reversing from $x'=43$]
Since $43\equiv1\pmod 6$, the output family is $\mathrm e$, so the valid types are \texttt{ee} or \texttt{oe}.
Choose the row $(s,j)=(\mathrm o,1)$ of type \texttt{oe} (token $\omega_1$), which has
$x'=12m+7$ (i.e.\ $\alpha=1$, $k=1$, $\delta=1$).
Compute
\[
m=\frac{(43-1)/6 - 1}{2^{1}}=\frac{7-1}{2}=3\in\mathbb{Z}.
\]
Then $p_6=5$ (since $s=\mathrm o$) and $x=18\cdot 3+6\cdot 1+5=65$.
Thus $U(43)=(3\cdot 43+1)/2=65$, matching the proposition.
\end{example}

\begin{corollary}[Mixed-$p$ version]
For a lifted row with parameter $p\ge 0$,
\[
x' \;=\; 6\!\left(2^{\alpha+6p} m + k_p\right)+\delta,
\qquad
k_p=\frac{64^{\,p}\beta+c}{9},
\]
the same construction with
\[
m \;=\; \frac{(x'-\delta)/6 - k_p}{2^{\alpha+6p}}
\]
yields a legal predecessor $x=18m+6j+p_6$ whenever $m\in\mathbb{Z}_{\ge 0}$, and
$3x'+1=2^{\alpha+6p}x$.
\end{corollary}
...
\fi
\newpage
\section*{Appendix E: Code and Data Availability}
