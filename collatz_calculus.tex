...
We model the inverse map as a discrete-time Markov chain. Assuming a stochastic driver where operations are chosen with equal probability ($p=0.5$), we compute the stationary distribution $\pi$.

\begin{table}[H]
\centering
\caption{Stationary Distribution ($\pi$) and Average Expansion.}
\label{tab:stationary}
\begin{tabular}{@{}l c c c c@{}}
\toprule
\textbf{State} & \textbf{Router} & \textbf{Prob ($\pi$)} & \textbf{Avg $\alpha$} & \textbf{Role} \\
\midrule
\textbf{0} & 0 & \textbf{0.275} & 3.0 & Dominant Attractor \\
\textbf{2} & 0 & 0.200 & 4.0 & Distributor \\
\textbf{5} & 1 & 0.150 & 2.0 & Odd Heart \\
\textbf{8} & 2 & 0.150 & 3.0 & Recycler \\
\textbf{3} & 1 & 0.125 & 5.0 & Twin of 0 \\
\textbf{6} & 2 & 0.100 & 4.0 & Transit \\
\bottomrule
\end{tabular}
\end{table}

\subsection{The Descent Chute}
The distribution highlights a critical topological constraint on geometric contraction. In the inverse map, contraction ($x' < x$) requires a slope $\alpha < \log_2 3$, which occurs only when $\alpha=1$. These parameters are topologically localized to exactly two edges: $8 \xrightarrow{A} 5$ and $5 \xrightarrow{B} 3$.

\begin{corollary}[The Descent Barrier]
Geometric contraction is not globally available; it is confined to the path $8 \to 5 \to 3$ (the ``Descent Chute''). Any globally bounded orbit must recurrently traverse this specific path with a frequency sufficient to counteract the mean expansion factor of $\approx 3.51$ per step derived from the stationary distribution.
\end{corollary}
