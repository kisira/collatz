...
\[
x \;=\; 18m + 6j + p_6,
\qquad m\in\mathbb Z_{\ge 0},\; j\in\{0,1,2\},\; p_6\in\{1,5\},
\]
so $p_6=1$ corresponds to $s=\mathrm e$ (i.e.\ $x\equiv 1\!\!\pmod 6$) and $p_6=5$ corresponds to $s=\mathrm o$ (i.e.\ $x\equiv 5\!\!\pmod 6$).
...
From $x=18m+6j+p_6$ we have $x\equiv 6j+p_6\pmod 9$.
A short check gives:

\begin{center}
\begin{tabular}{c|ccc}
 & $j=0$ & $j=1$ & $j=2$\\\hline
$s=\mathrm e$ ($p_6=1$): $x\bmod 9$ & $1$ & $7$ & $4$\\
$a_{\mathrm{bad}}$ in $\{0,2,4\}$ & $0$ & $2$ & $4$\\\hline
$s=\mathrm o$ ($p_6=5$): $x\bmod 9$ & $5$ & $2$ & $8$\\
...
By the Row Correctness Lemma (Lemma~\ref{lem:row-correctness} in your manuscript),
\[
...
\begin{enumerate}
    ...
    More precisely, writing $s:=v_2(A_{W'})$ and $A_{W'}=2^s u$ with $u$ odd, the congruence
    determines $m'$ uniquely modulo $2^{K-s}$; take the representative
    \[
    0 \le m' < 2^{K-s}.
    \]
...
representative in its congruence class), so that if $s:=v_2(A_W)$ then
\[
0\le m < 2^{K-s}.
\]
...
least nonnegative solution in its congruence class. Writing $s:=v_2(A_{W'})$, this yields
the canonical bound $0\le m' < 2^{K+1-s}$.
...
Meanwhile $y\equiv x\pmod{M_{K+1}}$ implies $y\equiv x\pmod 6$, hence $y\bmod 6=p_6$.
\end{proof}
...
\end{equation}
Let $s:=v_2(A_W)$ and write $A_W=2^s u$ with $u$ odd.

If the solutions are chosen compatibly, i.e.
\[
m_{K+1}\equiv m_K \pmod{2^{K-s}}\qquad (K\ge \max\{K_0,s+1\}),
\]
...
Moreover, if $u$ divides $\mathrm{RHS}/2^s$ as an ordinary integer (in particular,
if $u=1$, i.e.\ $A_W$ is a power of two), then $m\in\mathbb Z$ and the equality is exact in
$\mathbb Z$: $x_W(m)=x_{\mathrm{tar}}$.
\end{lemma}

\begin{proof}
From \eqref{eq:AW-congruence} and $v_2(A_W)=s$, it follows that for each $K>s$ the
congruence class of $m_K$ is unique modulo $2^{K-s}$: if $A_W m\equiv A_W m'\pmod{2^K}$,
then $2^s u(m-m')\equiv 0\pmod{2^K}$ and since $u$ is odd (hence invertible modulo $2^{K-s}$),
we get $m\equiv m'\pmod{2^{K-s}}$.
Thus compatibility means precisely that each $m_{K+1}$ refines $m_K$ modulo $2^{K-s}$,
...
To obtain an ordinary integer, reduce \eqref{eq:AW-congruence} modulo $2^s$.
Since $A_W m_K\equiv 0\pmod{2^s}$ for every $K$, we obtain $\mathrm{RHS}\equiv 0\pmod{2^s}$,
so $\mathrm{RHS}=2^s r$ for some $r\in\mathbb Z$.
...
Let $s\ge 0$ and let $\mathrm{RHS}\in\mathbb Z$. If there exists $m\in\mathbb Z_2$ such that
\[
2^s m \equiv \mathrm{RHS}\pmod{2^K}\quad\text{for all }K>s,
\]
then $v_2(\mathrm{RHS})\ge s$ and $m=\mathrm{RHS}/2^s\in\mathbb Z$.
\end{lemma}

\begin{proof}
Reducing the congruence modulo $2^s$ gives $\mathrm{RHS}\equiv 0\pmod{2^s}$, hence
$v_2(\mathrm{RHS})\ge s$. In $\mathbb Z_2$, division by $2^s$ is well-defined, so
$m=\mathrm{RHS}/2^s$. Since $\mathrm{RHS}$ is an integer divisible by $2^s$, the quotient
$\mathrm{RHS}/2^s$ is an ordinary integer.
\end{proof}
...
\begin{corollary}[Finite hit for $x$ once $(p_6,j)$ are fixed]
...
\]
Suppose that for each $K\ge 1$ your construction produces an odd integer of the form
\[
x_K \;:=\; 18m_K + 6j + p_6,
\]
...
Substituting into $x_K=18m_K+6j+p_6$ yields $x_K=18m+6j+p_6=x$ for all $K\ge K_0$.
\end{proof}
...
Write
\[
x = 18m + 6j + p_6,\qquad y = 18m' + 6j + p_6,
\]
...
Let $s:=v_2(A_W)$ for the witness word $W$ producing $x_K$. By the canonical choice in
the terminal solve step (Lemma~\ref{lem:lifting}, Step~5), $m_K$ is determined modulo
$2^{K-s}$ and we may choose it as the least nonnegative representative, so that
\[
0 \le m_K < 2^{K-s}.
\]
Choose $K$ large enough so that $K-s \ge K_0$ (equivalently $2^{K-s}\ge 2^{K_0}$).
...
Substituting into $x_K=18\widetilde m_K+6j+p_6$ yields $x_K=18m+6j+p_6=x$.
...
is the associated carry error (cf.\ the affine-plus-carry decomposition in
Section~\ref{sec:parameter-geometry}).
...
\href{https://doi.org/ DOI: 10.5281/zenodo.17993692}{Zenodo DOI: DOI: 10.5281/zenodo.17993692} and mirrored at
\href{git@github.com:kisira/collatz.git}{github.com/kisira/collatz}.
