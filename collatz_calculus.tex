...
\large{Deterministic Pathfinding and Magnitude Consumption on a 2-adic Manifold}}
...
\begin{abstract}
We introduce a constructive, automata-theoretic framework for analyzing the inverse dynamics of the Collatz $3x+1$ map. By modeling the system as a deterministic \textbf{Finite State Automaton (FSA)} acting on a modular network of 2nd-order 2-adic residue classes, we transform the reachability problem into a study of deterministic pathfinding on a rigid 2-adic manifold. First, we derive the \textbf{Unified $p$-lift Formula}, a deterministic mechanism that generates certified preimages while maintaining modular synchronization between target residues and step-count parity. We prove that this lifting mechanism is governed by an invariant \textbf{$4n+1$ Index Gear}, establishing a fundamental topological symmetry across the inverse tree.

Second, we implement \textbf{Algebraic Steering}, an algorithmic technique utilizing monotone padding sequences to satisfy arbitrary 2-adic linear constraints, facilitating a constructive proof of \textbf{Exact Reachability}. We establish that for every odd integer $x$, there exists an explicit, finite symbolic program connecting the root 1 to $x$. Within this framework, the Collatz map is shown to be a \textbf{deterministic magnitude consumer}, governed by a \textbf{Master Drift Identity} where the total arithmetic work required to reach the ground state is exactly $x-1$.

Finally, we analyze the probabilistic dynamics of the underlying Markov chain, computing a stationary distribution that identifies specific \textbf{``Descent Chutes''} responsible for orbital decay. To ensure absolute logical rigor and differentiate this work from heuristic approaches, the core algebraic lemmas, the steering engine , and the \textbf{Master Inverse Generator} have been formally verified using the \textbf{Rocq (Coq) proof assistant}.
\end{abstract}

\bigskip
\noindent \textbf{Keywords:} Collatz Conjecture; Finite State Automata; 2-adic Integers; Inverse Dynamics; Reachability; Formal Verification.
\medskip
\newpage
...
The structural integrity of the Master Inverse Generator relies on the specific cyclic behavior of powers of 2 within the modular residue system $\mathbb{Z}/9\mathbb{Z}$ . The residues of $2^n$ modulo 9 follow a deterministic sequence of period $k=6$ :
...
%The construction of the inverse Collatz tree is formalized through preimage arrays, where each cell represents a unique odd integer $x$ reached by the Unified $p$-lift Formula.
The construction of the inverse Collatz tree is formalized through preimage arrays generated by the inverse operator $x' = \frac{x \cdot 2^s - 1}{3}$. To ensure $x' \in \mathbb{Z}$, the cumulative 2-adic exponent $s$ is modularly synchronized with the residue of $x$: if $x \equiv 1 \pmod 6$, then $s \in \{2, 4, 6, \dots\}$, and if $x \equiv 5 \pmod 6$, then $s \in \{1, 3, 5, \dots\}$. Unlike the Unified $p$-lift Formula, which filters specifically for steerable preimages ($x' \not\equiv 0 \pmod 3$), these arrays map the complete 2nd-order 2-adic manifold. This includes terminal branches (integers divisible by 3) which exist as nodes in the global tree but do not permit further odd branching. These arrays demonstrate that the lifting mechanism is governed by the universal $4n+1$ recurrence:
\begin{equation}
    a_{n} = 4a_{n-1} + 1
\end{equation}
...
\newblock Zenodo, 2025. DOI: 10.5281/zenodo.18142088.
...
% -------------------------------------------------------------------------
% APPENDIX : DERIVATION OF THE FORWARD IDENTITY
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
% APPENDIX : WITNESS TABLES AND TECHNICAL METRICS
% -------------------------------------------------------------------------
...
\section{Modular Synchronization of the Inverse Map}
\label{appendix:modular-sync}

For the inverse Collatz operator $x' = (x \cdot 2^s - 1)/3$ to produce an integer preimage $x' \in \mathbb{Z}$, the numerator must be divisible by 3. This requirement necessitates a specific synchronization between the residue of the target $x$ and the parity of the 2-adic exponent $s$.

\subsection{The Divisibility Constraint}
The condition $x' \in \mathbb{Z}$ is equivalent to the modular congruence:
\begin{equation}
    x \cdot 2^s - 1 \equiv 0 \pmod 3 \implies x \cdot 2^s \equiv 1 \pmod 3
\end{equation}
Because we consider only odd $x$, the target must satisfy $x \equiv 1 \pmod 6$ or $x \equiv 5 \pmod 6$. These residues map to $x \equiv 1 \pmod 3$ and $x \equiv 2 \pmod 3$, respectively.

\subsection{Parity of the Exponent $s$}
The powers of 2 modulo 3 follow a cyclic pattern of period 2:
\begin{equation}
    2^s \pmod 3 =
    \begin{cases}
      1 & \text{if } s \text{ is even} \\
      2 & \text{if } s \text{ is odd}
    \end{cases}
\end{equation}

\subsection{Case Synchronization}
Substituting these values into the modular constraint $x \cdot 2^s \equiv 1 \pmod 3$:

\begin{enumerate}
    \item \textbf{Case $x \equiv 1 \pmod 6$:} Here $x \equiv 1 \pmod 3$. The constraint becomes $(1) \cdot 2^s \equiv 1 \pmod 3$. This is satisfied only when \textbf{$s$ is even} ($s \in \{2, 4, 6, \dots\}$).

    \item \textbf{Case $x \equiv 5 \pmod 6$:} Here $x \equiv 2 \pmod 3$. The constraint becomes $(2) \cdot 2^s \equiv 1 \pmod 3$. Since $2 \cdot 2 = 4 \equiv 1 \pmod 3$, we require $2^s \equiv 2 \pmod 3$. This is satisfied only when \textbf{$s$ is odd} ($s \in \{1, 3, 5, \dots\}$).
\end{enumerate}



This synchronization ensures that every row in the Preimage Arrays (Section 2.4) consists strictly of integers, establishing the 2nd-order 2-adic manifold as the continuous substrate of the Collatz tree.
...
\href{https://doi.org/10.5281/zenodo.18142088}{Zenodo DOI: 10.5281/zenodo.18142088} and mirrored at
\href{https://github.com/kisira/collatz}{github.com/kisira/collatz}.


% -------------------------------------------------------------------------
% APPENDIX : FORMALIZATION INDEX (COQ)
% -------------------------------------------------------------------------
