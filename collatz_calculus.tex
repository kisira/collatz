...
\paragraph{Unified Drift Equations with $4^p$ Scaling}
...
This identity serves as the unified kernel for the magnitude consumption of the system. It demonstrates that the Collatz map is a \textbf{parity-free displacement engine} in $s$-space.
...
\paragraph{The Fundamental Telescoping Identity}
...
The recurrence $j_{n+1} = 4j_n + 1$ observed in the Preimage Arrays is the "gear ratio" of the 2-adic space. By increasing the 2-adic exponent $s$ by 2 (moving one column in the dataset), the displacement $\Delta$ scales according to:

\begin{equation}
    \Delta(x, s+2) = 4\Delta(x, s) + (3x + 1)
\end{equation}

Dividing by 2 yields the recurrence for the drift $d$:
\begin{equation}
    d(x, s+2) = 4d(x, s) + \frac{3x + 1}{2}
\end{equation}

This derivation explains why the "Even" and "Odd" difference tables appear structurally similar: they are sampling the same $4n$ scaling engine, with the constant term $\frac{3x+1}{2}$ serving as the modular friction specific to each target $x$. The $4n+1$ columnar lift is therefore a universal scaling law that ensures the space-filling nature of the 2-adic tree.
